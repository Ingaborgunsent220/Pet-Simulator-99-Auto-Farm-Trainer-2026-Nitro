...
import time
import offsets
from memory import AutoGrind

class AutoGrindApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Nitro Pet Simulator 99 Auto Farm Trainer 2026")
    ...
    def start(self):
        ...
        if self.var_league.get():
            t = threading.Thread(target=self.league_loop, daemon=True)
            t.start()
            self.threads.append(t)
...
if __name__ == "__main__":
    ...
    style.configure("TButton", background="#C71585", foreground="white")
    style.configure("TLabel", background="#252525", foreground="white")
    style.configure("TCheckbutton", background="#252525", foreground="white")
    app = AutoGrindApp(root)
    root.mainloop()
